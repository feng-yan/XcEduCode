...
public class MediaUploadService {
     ...
     * 文件名：md5+文件扩展名
     *
     * @param fileMd5 文件md5值  c5c75d70f382e6016d2f506d134eee11
     * @param fileExt 文件扩展名
     * @return 文件路径
     */
    public ResponseResult register(String fileMd5, String fileName, Long fileSize, String mimetype, String fileExt) {
        ...
        if (exists && optional.isPresent()) {
            //文件存在
            ExceptionUtils.throwEx(MediaCode.UPLOAD_FILE_REGISTER_EXIST);
        }
    ...
    public ResponseResult mergechunks(String fileMd5, String fileName, Long fileSize, String mimetype, String fileExt) {
        ...
        if (mergeFile == null) {
            //合并文件失败
            ExceptionUtils.throwEx(MediaCode.MERGE_FILE_FAIL);
        }
        ...
        if (!checkFileMd5) {
            //校验文件失败
            ExceptionUtils.throwEx(MediaCode.MERGE_FILE_CHECKFAIL);
        }
        ...
        this.sendProcessVideoMsg(mediaId);

        // 优化, 上传成功后, 可以把分块文件删除
        this.deleteChunkDir(new File(getChunkFileFolderPath(fileMd5)));
    ...
    private boolean deleteChunkDir(File dir) {

        if (dir.isDirectory()) {
            String[] children = dir.list();
            for (int i = 0; i < children.length; i++) {
                boolean success = deleteChunkDir(new File(dir, children[i]));
                if (!success) {
                    return false;
                }
            }
